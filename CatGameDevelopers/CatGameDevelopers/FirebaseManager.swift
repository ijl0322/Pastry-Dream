...
class FirebaseManager {
    ...
    func addLevelToFirebase(tiles: [[Int]], timeLimit: Int) {
        let newLevelIndex = AllLevels.shared.numberOfLevels
        // This is numberOfLevels not numberOfLevels + 1
        // because in firebase the array of levels' index start at 0
        levelsRef.child("\(newLevelIndex)").setValue(["tiles" : tiles, "timeLimit": timeLimit])
    }
